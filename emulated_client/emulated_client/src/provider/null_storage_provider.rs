...
pub struct OpenMlsRustNullStorageCrypto {
    crypto: RustCrypto,
    storage: NullStorageProvider,
}

impl OpenMlsProvider for OpenMlsRustNullStorageCrypto {
    type CryptoProvider = RustCrypto;
    type RandProvider = RustCrypto;
    type StorageProvider = NullStorageProvider;
...
pub struct NullStorageProvider {
    values: std::sync::RwLock<std::collections::HashMap<Vec<u8>, Vec<u8>>>,
}
...
impl StorageProvider<CURRENT_VERSION> for NullStorageProvider {
...
fn epoch_key_pairs_id(
    ...
    leaf_index: u32,
) -> Result<Vec<u8>, <NullStorageProvider as StorageProvider<CURRENT_VERSION>>::Error> {
