...
impl GossipSubBroker {
    ...
    pub fn publish_group_info(&self, group_name: String, group_info: GroupInfoAndTree) -> Result<(), String> {
        ...
        match response {
            ...
            Err(e) => {
                if e.contains("NotFound") {
                    ...
                    tracing::error!("Error checking group existence: {}", e);
                    Err(e)
                }
            }
        }
    ...
    pub fn group_info(&self, group_name: String) -> Result<GroupInfoAndTree, String> {
        ...
        let group_info = GroupInfoAndTree::tls_deserialize(&mut response.as_slice()).map_err(|e| format!("Error deserializing group info: {:?}", e))?;
    ...
    pub async fn consume_key_package(&self, group_name: String) -> Result<KeyPackageIn, String> {
        ...
        tracing::info!("Consuming key package for {}", key);
        ...
        let response =
            async_std::future::timeout(Duration::from_secs(5), receiver)
                .await.map_err(|e| format!("Timeout on request: {:?}", e))?
                .map_err(|e| format!("Error receiving from channel: {:?}", e))??;
        tracing::info!("Key package obtained");
