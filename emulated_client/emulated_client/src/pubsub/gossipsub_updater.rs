...
use std::thread;
use crate::config::{Directory, GossipSubConfig};
...
pub struct GossipSubUpdater<P: OpenMlsProvider> {
    //user_name: String,
...
impl<P: OpenMlsProvider + std::marker::Send + 'static> GossipSubUpdater<P> {
    pub fn new(users: HashMap<String, Arc<Mutex<User<P>>>>, config: GossipSubConfig, rx: Receiver<GossipSubQueueMessage>) -> Self {
        ...
        match msg {
            GossipSubQueueMessage::Message(topic_str, message) => {
                ...
                match swarm.behaviour_mut().gossipsub.publish(topic.clone(), message.clone()) {
                    Err(e) => {
                        tracing::error!("Error publishing message to topic {}: {:?}", topic_str, e);
                    }
                    _ => {}
                }
            ...
            GossipSubQueueMessage::Publish(_name, key, value) => {
                tracing::info!("Publishing record with key: {key}");
            ...
            GossipSubQueueMessage::Get(key, response, remove) => {
                if let Directory::Kademlia = self.config.directory {
                    ...
                    //tracing::info!("Requesting record with key: {key}", self.user_name);
                ...
                if let Some((sender, remove_entry)) = self.pending_get_requests.remove(&id) {
                    match result {
                        ...
                        QueryResult::GetRecord(Ok(GetRecordOk::FoundRecord(PeerRecord { record, .. }))) => {
                            ...
                            if remove_entry {
                                if value.is_empty() {
                                    //tracing::info!("Record not found: {:?}", key);
