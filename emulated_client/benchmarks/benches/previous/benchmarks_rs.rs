...
use rand::{Rng};
type Member = (usize, Group<dyn MlsConfig>);
            ...
            c.bench_function(
                ...
                |b| {
                    b.iter(|| {
                       ...
                       committer_group
                            .commit_builder()
                            .add_member(key_package.clone()).unwrap()
                            .build().unwrap();

                            
                        committer_group.clear_pending_commit();
                    });
                }
            );
            c.bench_function(
                ...
                |b| {
                    b.iter_batched(
                        || {
                            ...
                            let (_, committer_group) = active_members.get_mut(chosen_committer).unwrap();
                            //let committer_identity = identities.get(*committer_id).unwrap();
                            let key_package = new_member_identity.generate_key_package_message(Default::default(), Default::default(), None).unwrap();
                        ...
                        |welcome | {
                            new_member_identity.join_group(None, &welcome.welcome_messages[0], None).unwrap();
                        },
                    ...
                    b.iter_batched(
                        || {
                            ...
                            let mut chosen_processer = {
                                ...
                                let (_processer_id, group) = active_members.get_mut(sample).unwrap();

                                group.clone()
                            };
                            (commit, chosen_processer)
                        },
                        |(commit, mut chosen_processer) | {
                            //let mut active_members = active_members.borrow_mut();
                            //let (_processer_id, group) = active_members.get_mut(chosen_processer).unwrap();

                            chosen_processer.process_incoming_message(commit.commit_message.clone()).unwrap();
                        },
                        BatchSize::LargeInput
                    );
                }
