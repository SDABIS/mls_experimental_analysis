...
use openmls::prelude::{key_package_in::*, tls_codec, KeyPackage, MlsMessageIn, RatchetTreeIn};
...
pub enum DSMessage {
    ...
    WelcomeAcknowledgement(WelcomeAcknowledgement),
}
...
pub struct WelcomeAcknowledgement {
    pub sender: String,
    pub group: String,
}
...
pub struct GroupInfoAndTree {
    pub group_info: VerifiableGroupInfo,
    pub ratchet_tree: Option<RatchetTreeIn>,
}
...
pub struct GroupMessage {
    ...
    pub active_members: Option<Vec<TlsByteVecU32>>,
}

impl GroupMessage {
    /// Create a new `GroupMessage` taking an `MlsMessageIn` and slice of
    /// recipient names.
    pub fn new(msg: MlsMessageIn, sender: Vec<u8>, ratchet_tree: Option<RatchetTreeIn>, recipients: &[Vec<u8>], active_members: Option<Vec<Vec<u8>>>) -> Self {
        Self {
            msg,
            sender: sender.into(),
            recipients: recipients
                ...
                .into(),
                ratchet_tree,
            active_members: match active_members {
                Some(members) => Some(members.iter().map(|r| r.clone().into()).collect()),
                None => None,
            }
        }
    }
...
impl GroupInfoAndTree {
    pub fn new(group_info: VerifiableGroupInfo, ratchet_tree: Option<RatchetTreeIn>) -> Self {
        Self {
            group_info,
            ratchet_tree,
        }
...
impl tls_codec::Size for WelcomeAcknowledgement {
    fn tls_serialized_len(&self) -> usize {
        TlsByteVecU8::from_slice(self.group.as_bytes()).tls_serialized_len() + TlsByteVecU8::from_slice(self.group.as_bytes()).tls_serialized_len()
    }
}

impl tls_codec::Serialize for WelcomeAcknowledgement {
    fn tls_serialize<W: std::io::Write>(&self, writer: &mut W) -> Result<usize, tls_codec::Error> {
        let mut written = TlsByteVecU8::from_slice(self.sender.as_bytes()).tls_serialize(writer)?;
        written += TlsByteVecU8::from_slice(self.group.as_bytes()).tls_serialize(writer)?;
        Ok(written)
    }
}

impl tls_codec::Deserialize for WelcomeAcknowledgement {
    fn tls_deserialize<R: std::io::Read>(bytes: &mut R) -> Result<Self, tls_codec::Error> {
        let sender = String::from_utf8(TlsByteVecU8::tls_deserialize(bytes)?.into_vec()).unwrap();
        let group = String::from_utf8(TlsByteVecU8::tls_deserialize(bytes)?.into_vec()).unwrap();
        Ok(Self { sender, group })
    }
}
...
impl tls_codec::Size for GroupMessage {
    fn tls_serialized_len(&self) -> usize {
        self.msg.tls_serialized_len() + self.sender.tls_serialized_len() + self.ratchet_tree.tls_serialized_len() + self.recipients.tls_serialized_len() + self.active_members.tls_serialized_len()
    }
}
...
impl tls_codec::Serialize for GroupMessage {
    fn tls_serialize<W: std::io::Write>(&self, writer: &mut W) -> Result<usize, tls_codec::Error> {
        ...
        written += self.ratchet_tree.tls_serialize(writer)?;
        written += self.recipients.tls_serialize(writer)?;
        written += self.active_members.tls_serialize(writer)?;
...
impl tls_codec::Deserialize for GroupMessage {
    fn tls_deserialize<R: std::io::Read>(bytes: &mut R) -> Result<Self, tls_codec::Error> {
        ...
        let ratchet_tree = Option::<RatchetTreeIn>::tls_deserialize(bytes)?;
        let recipients = TlsVecU32::<TlsByteVecU32>::tls_deserialize(bytes)?;
        let active_members = Option::<Vec<TlsByteVecU32>>::tls_deserialize(bytes)?;
        Ok(Self { msg, sender, ratchet_tree, recipients, active_members })
    }
}

impl tls_codec::Size for GroupInfoAndTree {
    fn tls_serialized_len(&self) -> usize {
        self.group_info.tls_serialized_len() + self.ratchet_tree.tls_serialized_len()
    }
}

impl tls_codec::Serialize for GroupInfoAndTree {
    fn tls_serialize<W: std::io::Write>(&self, writer: &mut W) -> Result<usize, tls_codec::Error> {
        let mut written = self.group_info.tls_serialize(writer)?;
        written += self.ratchet_tree.tls_serialize(writer)?;
        Ok(written)
    }
}

impl tls_codec::Deserialize for GroupInfoAndTree {
    fn tls_deserialize<R: std::io::Read>(bytes: &mut R) -> Result<Self, tls_codec::Error> {
        let group_info = VerifiableGroupInfo::tls_deserialize(bytes)?;
        let ratchet_tree = Option::<RatchetTreeIn>::tls_deserialize(bytes)?;
        Ok(Self { group_info, ratchet_tree })
    }
}