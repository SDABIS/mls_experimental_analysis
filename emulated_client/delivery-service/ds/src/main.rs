...
pub struct DsData {
    ...
    group_info: Mutex<HashMap<Vec<u8>, Option<GroupInfoAndTree>>>,
}
    ...
    let group_info = match groups.get(&id) {
        Some(c) => {
            match c {
                ...
                None => {
                    log::debug!("{username} requests group info of {:?}: Forbidden", std::str::from_utf8(id.as_slice()).unwrap());
...
async fn post_group_info(
    ...
    let group_info = match GroupInfoAndTree::tls_deserialize(&mut &bytes[..]) {
        Ok(gi) => gi,
        Err(_) => {
            log::error!(
...
async fn main() -> std::io::Result<()> {
    ...
    if group_info_mutex {
        log::info!("Group Info Mutex is enabled");
    } else {
        log::info!("Group Info Mutex is disabled");
    }
